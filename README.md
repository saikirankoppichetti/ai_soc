...
This installation uses "--state-dir ../../work/runtime" for every native command.
Its administrator credentials are in "../../work/runtime/admin-credentials.txt".
Use the same state directory when restarting or backing up. No automatic
login/startup service is installed.
...
validation and all ten application/maintenance image builds. Four Chroma advisories
have narrowly scoped, versioned, expiring mitigations because the affected
HTTP/configurable-embedding features are absent from this deployment. New or
expired advisories fail the audit. See [the exception rationale](docs/security/dependency-exceptions.json).

[Apache 2.0 license](LICENSE).

## Maintainer

AI-SOC is maintained by Sai Kiran Koppichetti, an AI/ML Engineer with over 5 years
of experience in machine learning, data science, and analytics. His work focuses
on building end-to-end RAG and agentic pipelines, including ingestion, chunking,
embedding, and evaluation for regulated domains such as healthcare and financial
services.
